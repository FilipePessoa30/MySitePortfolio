...
import { AnimatedGear, Icon, ScrollReveal, useToast } from './portfolio-kit';
    ...
    <section style={{ position: 'relative', padding: '120px 32px', background: '#1a1a1a', color: '#fff' }} data-screen-label="Contact">
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 64 }}>
        ...
        <ScrollReveal as="form" distance={40} delay={100} onSubmit={onSubmit} style={{ padding: 32, background: '#0f0f0f', border: '1px solid #2a2a2a', borderRadius: 18 }}>
          ...
          <button type="submit" disabled={busy} className="fp-shimmer-btn" style={{ width: '100%', padding: '14px 20px', background: busy ? '#2a2a2a' : '#00c853', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: 'var(--fp-font-body)', cursor: busy ? 'wait' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .2s' }}>
