...
export const Icon = ({ name, size = 20, ...rest }: { name: string; size?: number; [key: string]: any }) => {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" className={cls} style={{ opacity }} aria-hidden="true">
      <g fill={color}>
        ...
        {teeth.map((_, i) => (
          <rect key={i} x="47" y="2" width="6" height="15" transform={`rotate(${i * 30} 50 50)`} opacity={opacity * 2 + 0.2} />
        ))}
...
export const StatusPill = ({ text, color = '#1e4e8c' }: { text: string; color?: string }) => (
...
export const SectionHeading = ({ kicker, title, bullet = '#1e4e8c' }: { kicker?: string; title: string; bullet?: string }) => (
...
export const TechnicalGrid = ({ style = {} }: { style?: React.CSSProperties }) => (
  <div className="fp-technical-grid" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, ...style }} />
);

export const useToast = () => {
  const [msg, setMsg] = React.useState<{ text: string; tone: 'success' | 'error'; id: number } | null>(null);
