// Hero.jsx — reversible scroll-linked hero with role-led palette
const Hero = ({ onCtaProjects, onCtaContact }) => {
  const [videoReady, setVideoReady] = React.useState(false);
  const sectionRef = React.useRef(null);
  const [progress, setProgress] = React.useState(0); // 0 = fully in view, 1 = fully scrolled past top
  ...
  React.useEffect(() => {
    const onScroll = () => {
      ...
      // progress goes 0 → 1 as the section's top scrolls from 0 to -window.innerHeight
  ...
  // Reversible scroll-linked transforms — every value is a function of `progress`,
  // so scrolling up automatically returns elements to their original state.
  const fadeOut       = Math.max(0, 1 - progress * 1.6);            // text fades
  const parallaxUp    = (v) => `translateY(${-v * progress}px)`;    // drift upward
  const parallaxDown  = (v) => `translateY(${ v * progress}px)`;    // drift downward
  ...
  return (
    <section ref={sectionRef}
      ...
      {/* Video backdrop — scales slightly as you scroll for a subtle parallax */}
      ...
      {/* Decorative gears — drift at different speeds */}
      ...
      {/* Content grid — content drifts up + fades as you scroll down, returns on the way back */}
      <div style={{
        ...
        {/* Left — copy (no more avatar lockup; navbar already has it) */}
            ...
            {[
              ...
              <a key={s.icon} href={s.href} target="_blank" rel="noopener noreferrer"
                 ...
                 style={{
                   ...
                   color:'var(--fp-fg-1)',textDecoration:'none',
                   transition:'transform .18s var(--fp-ease-snap), box-shadow .18s, color .18s, border-color .18s'
                 }}
                 onMouseEnter={e => { e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.color='#1e4e8c'; e.currentTarget.style.borderColor='#1e4e8c'; e.currentTarget.style.boxShadow='0 0 18px rgba(30,78,140,0.25)'; }}
                 onMouseLeave={e => { e.currentTarget.style.transform=''; e.currentTarget.style.color=''; e.currentTarget.style.borderColor=''; e.currentTarget.style.boxShadow=''; }}>
        ...
        {/* Right — portrait with breathing glow; drifts down slightly, fades */}
        <div className="fp-fade-up d-300 hero-portrait"
             style={{
               position:'relative',width:'min(420px, 38vw)',aspectRatio:'3/4',
               opacity: fadeOut,
               transform: parallaxDown(60),
             }}>
          <div className="fp-breathe" style={{
            position:'absolute',inset:-40,
            background:'radial-gradient(closest-side, rgba(30,78,140,0.35), transparent 70%)',
            filter:'blur(28px)',zIndex:0
          }}/>
          <div style={{
            position:'absolute',inset:-2,borderRadius:28,
            background:'linear-gradient(135deg, #1e4e8c 0%, #0d2648 30%, #00c853 60%, #8b1e3f 100%)',
            backgroundSize:'200% 200%',animation:'fp-fade-in 1s .4s both, fpBorderShift 10s ease-in-out infinite',
            zIndex:1,opacity:.85
          }}/>
          <div style={{
            position:'relative',zIndex:2,width:'100%',height:'100%',borderRadius:26,overflow:'hidden',
            boxShadow:'0 30px 60px -20px rgba(0,0,0,0.3), 0 8px 16px -4px rgba(0,0,0,0.1)',
            background:'#000'
          }}>
            <img src="../../assets/filipe-secondary.jpg" alt="Filipe Pessoa Sousa"
                 style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 22%'}}/>
          </div>

          {/* Floating tech-icon badges */}
          {[
            { key:'py', top:'-3%',   left:'-7%',  delay:'1.0s', bg:'#1a1a1a', label:'Python',
              svg:(<svg viewBox="0 0 128 128" width="24" height="24" aria-hidden="true">
            ...
            { key:'hf', top:'9%',    right:'-7%', delay:'1.15s', bg:'#0f0f0f', label:'HuggingFace',
              svg:(<svg viewBox="0 0 95 88" width="22" height="22" aria-hidden="true"><path fill="#FF9D00" d="M47.21 76.07c19.64 0 35.56-15.92 35.56-35.56S66.85 4.95 47.21 4.95 11.65 20.87 11.65 40.51s15.92 35.56 35.56 35.56Z"/><path fill="#FFD21E" d="M81.49 52.74c-.32-1.23-1.39-2.13-2.66-2.26a3.17 3.17 0 0 0-3.27 2.17c-3.57 10.7-11.62 19.1-21.33 23.06a27.72 27.72 0 0 1-17.11 1.2c-9.07-2.2-17.32-8.63-22.33-17.05a3.17 3.17 0 0 0-3.62-1.42 3.18 3.18 0 0 0-2.29 3.14c.05 1.42.42 2.38.89 3.19 5.97 10.04 15.77 17.69 26.55 20.32 6.85 1.67 14.25 1.19 21.03-1.58 11.62-4.74 21.2-14.79 25.41-27.42.32-.95.77-2.31.73-3.35Z"/><circle cx="32.13" cy="39.69" r="5" fill="#3A3B45"/><circle cx="62.15" cy="39.69" r="5" fill="#3A3B45"/><path fill="#3A3B45" d="M55.68 56.56c-.18 3.09-4.31 5.5-8.34 5.37-4.02-.13-7.94-2.76-7.76-5.85.18-3.09 4.2-2.87 8.23-2.74 4.02.13 8.05.14 7.87 3.22Z"/></svg>) },
            { key:'bi', bottom:'26%', right:'-9%', delay:'1.3s', bg:'#1a1a1a', label:'Power BI',
              svg:(<svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true"><defs><linearGradient id="fpBi" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#FFD34F"/><stop offset="1" stopColor="#E8A81C"/></linearGradient></defs><path fill="url(#fpBi)" d="M9 3h4a1 1 0 0 1 1 1v17H8V4a1 1 0 0 1 1-1zm7 5h3a1 1 0 0 1 1 1v12h-4V8zM4 13h3a1 1 0 0 1 1 1v7H3v-7a1 1 0 0 1 1-1z"/></svg>) },
            { key:'tf', bottom:'-4%', left:'-7%', delay:'1.45s', bg:'#0f0f0f', label:'TensorFlow',
              svg:(<svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true"><path fill="#FF6F00" d="M1.292 5.856 11.54 0v24l-4.095-2.378V7.603l-6.168 3.564.015-5.31zm21.43 5.311-.014-5.31L12.46 0v24l4.095-2.378V14.87l3.092 1.788-.018-4.77-3.074-1.759V7.603l6.168 3.564z"/></svg>) },
          ].map((b, i) => (
      ...
      <style>{`
        ...
        .fp-float-badge { transition: transform .2s var(--fp-ease-snap), box-shadow .2s; }
